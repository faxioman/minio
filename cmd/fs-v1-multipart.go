...
func (fs *FSObjects) getMultipartSHADir(bucket, object string) string {
	return pathJoin(fs.fsPath, bucket, minioMetaMultipartBucket, getSHA256Hash([]byte(pathJoin(bucket, object))))
}
...
func (fs *FSObjects) backgroundAppend(ctx context.Context, bucket, object, uploadID string) {
	...
	if file == nil {
		file = &fsAppendFile{
			filePath: pathJoin(fs.fsPath, bucket, minioMetaTmpBucket, fs.fsUUID, fmt.Sprintf("%s.%s", uploadID, mustGetUUID())),
		}
